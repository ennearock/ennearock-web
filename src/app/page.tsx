  ...
  return <>
    ...
    <main>
      <section className="hero-section"><Container className="hero-grid">
        <div className="hero-copy"><Eyebrow>{content.hero.eyebrow}</Eyebrow><h1><Lines text={content.hero.title} /><br /><em><Lines text={content.hero.accentTitle} /></em></h1><p>{content.hero.description}</p><div className="hero-actions"><Button arrow href={content.hero.primaryCtaHref} variant="lime">{content.hero.primaryCtaLabel}</Button><Button href={content.hero.secondaryCtaHref} variant="ghost">{content.hero.secondaryCtaLabel} <ArrowUpRight size={17} /></Button></div><div className="hero-proof"><div className="avatar-stack">{content.hero.avatars.map((avatar, index) => <span key={`${avatar}-${index}`}>{avatar}</span>)}</div><p><strong>{content.hero.proofValue}</strong><br />{content.hero.proofText}</p></div></div>
